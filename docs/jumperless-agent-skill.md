# Jumperless V5 Agent Skill

> **Canonical source:** [Architeuthis-Flux/Large-Breadboard-Model](https://github.com/Architeuthis-Flux/Large-Breadboard-Model) (Agent Skill + `mcp/` submodule)
> **Download:** [GitHub `SKILL.md`](https://github.com/Architeuthis-Flux/Large-Breadboard-Model/blob/main/SKILL.md)
> **MCP:** build from the `mcp/` submodule in that repo (see the [Automation overview](07.5-automation.md))
> **Install:** clone into `~/.cursor/skills/jumperless-v5` or `~/.claude/skills/jumperless-v5` with `--recurse-submodules`

This is a copy of the [Agent Skills](https://agentskills.io/) `SKILL.md` for the
Jumperless V5, for readers browsing the docs site. The **canonical, maintained**
version is in the repo linked above — re-clone from there to get the full
`reference/` tree and the `mcp/` server submodule, which are not reproduced on
this page.
