...
Connects two nodes while **skipping LED computation**.

This function adds connections without updating LED state. It's not significantly faster overall, but it's useful when making many connections at once - you can defer LED updates until all connections are done.

*   `node1`, `node2`: The nodes to connect. Can be integers, strings, or constants.
*   `duplicates` (optional): Same behavior as `connect()` - see above for details.

**Example:**
```jython
# Make multiple connections without LED updates between each one
for i in range(1, 11):
    fast_connect(i, i+10)
# LEDs update automatically when loop completes

# With duplicate management
fast_connect(1, 5, duplicates=2)  # Force 2 parallel paths, skip LED update
```

### `fast_disconnect(node1, node2)`
Disconnects two nodes while **skipping LED computation**.

Same LED-skipping behavior as `fast_connect()`. Useful for bulk disconnections.

**Example:**
```jython
# Bulk disconnections without LED updates
for i in range(1, 11):
    fast_disconnect(i, i+10)
# LEDs update when loop completes
``` -->
...
*   Understanding chip utilization
*   Verifying duplicate path behavior
*   Educational purposes (seeing how routing works internally)
...
Turn any routable node into a digital output with custom voltage levels. Fake GPIO solves two key limitations of the RP2350's hardware GPIO:

1. **Only 10 routable GPIO pins** → Fake GPIO lets you use any routable node (breadboard holes, Arduino pins, special nodes)
2. **Only 3.3V logic levels** → Fake GPIO supports -8V to +8V for industrial protocols

### Why Use Fake GPIO?

**More Pins:**
The Jumperless has 10 routable GPIO pins (GPIO_1 through GPIO_8, plus UART). When you need more digital outputs, use Fake GPIO on any routable node - breadboard holes, Arduino pins, ADC/DAC nodes, etc.

**Custom Voltages:**
Many industrial protocols and devices require voltages the RP2350 can't provide:
- **RS-485**: ±8V differential signaling
- **Legacy TTL**: 5V logic for older chips
- **Industrial sensors**: 5V or 8V logic levels
- **Bipolar signals**: Switch between positive and negative voltages (±8V range)

### `FakeGpioPin(node, [mode], [v_high], [v_low], [threshold_high], [threshold_low])`

Creates a virtual GPIO pin on any routable node.

**Parameters:**
- `node` (int, required): Any routable node - breadboard holes (1-60), Arduino pins (D0-D13, A0-A7), special nodes, etc.
- `mode` (int, optional): `OUTPUT` or `INPUT` (default: `OUTPUT`)
...
**Raises:** `FakeGpioDisconnect` exception if configuration fails
...
# Simple 5V/0V output on breadboard hole
led = j.FakeGpioPin(25)
led.on()    # HIGH (5V)
led.off()   # LOW (0V)

# RS-485 differential pair (±8V)
rs485_a = j.FakeGpioPin(20, j.OUTPUT, 8.0, -8.0)
rs485_b = j.FakeGpioPin(21, j.OUTPUT, -8.0, 8.0)

rs485_a.on()   # A: +8V, B: -8V (differential high)
rs485_a.off()  # A: -8V, B: +8V (differential low)

# 5V industrial sensor power
sensor = j.FakeGpioPin(30, j.OUTPUT, 5.0, 0.0)
sensor.on()  # 5V output

# Use any routable node - breadboard, Arduino pins, etc.
pins = [
    j.FakeGpioPin(15),   # Breadboard hole
    j.FakeGpioPin(j.D7), # Arduino pin
    j.FakeGpioPin(j.A0), # Arduino analog pin
    j.FakeGpioPin(j.DAC0), # DAC node
]
...
#### `pin.on()`
Sets the pin to HIGH state (v_high voltage).

#### `pin.off()`
Sets the pin to LOW state (v_low voltage).

#### `pin.toggle()`
Toggles the pin between HIGH and LOW states.

#### `pin.value([val])`
Gets or sets the pin value (0 or 1).
...
```jython
j.OUTPUT              # Output mode
j.INPUT               # Input mode (for future use)
```
...
```jython
# RS-485 requires ±8V differential pair
tx_a = j.FakeGpioPin(20, j.OUTPUT, 8.0, -8.0)
tx_b = j.FakeGpioPin(21, j.OUTPUT, -8.0, 8.0)

# Send differential bits
def send_bit(bit):
    if bit:
        tx_a.on()   # A: +8V, B: -8V (differential +16V)
    else:
        tx_a.off()  # A: -8V, B: +8V (differential -16V)
    tx_b.value(1 - tx_a.value())  # Opposite of A
```

#### Multiple Output Pins
```jython
# Need more than 10 GPIO? Use any routable nodes!
outputs = [
    j.FakeGpioPin(15),      # Breadboard hole
    j.FakeGpioPin(16),      # Breadboard hole
    j.FakeGpioPin(j.D7),    # Arduino pin
    j.FakeGpioPin(j.A0),    # Arduino analog pin
]

# Turn any routable node into GPIO
for pin in outputs:
    pin.on()
```

#### Industrial 5V/8V Logic
```jython
# 5V sensor enable
sensor_enable = j.FakeGpioPin(25, j.OUTPUT, 5.0, 0.0)
sensor_enable.on()  # 5V output

# 8V device control
device = j.FakeGpioPin(26, j.OUTPUT, 8.0, 0.0)
device.on()  # 8V to energize device
```

#### 5V TTL for Legacy Chips
```jython
# Some older ICs need 5V logic (not 3.3V)
ttl_output = j.FakeGpioPin(30, j.OUTPUT, 5.0, 0.0)
ttl_output.on()  # 5V HIGH (not 3.3V)
```

### Limitations

- **DAC Limit**: Maximum 2 custom voltage levels at once
- **Protected pins**: Cannot use hardware GPIO pins (GPIO_20-GPIO_27) or UART
- **Voltage range**: -8V to +8V maximum
- **Works with most nodes**: Any routable node can be used (breadboard, Arduino pins, special nodes)

### Troubleshooting

**Configuration fails with "No available DAC":**
Both DACs are in use. Use standard voltages (5V, 3.3V, 0V) or free up a DAC.

**"Cannot use RP_GPIO pins":**
You tried to use hardware GPIO pins (GPIO_20-GPIO_27). Use any other routable node instead (breadboard holes, Arduino pins, special nodes)
...
### `cycle_term_color([reset=False], [step=100.0], [flush=True])`
Cycles through the terminal color palette.

*   `reset` (optional): If `True`, reset to start of color sequence
*   `step` (optional): Color increment step (default: 100.0)
*   `flush` (optional): Flush output immediately (default: `True`)

Useful for creating rainbow effects or visually distinguishing output sections.
...
cycle_term_color(reset=True, step=5.0)
```
